...
import { notifyChannel } from "../agent/notifier.js";
...
function formatDigestTelegramText(input: {
  title: string;
  rationale: string;
  risks: string[];
  benefits: string[];
  ...
  const lines = [
    `Briefing Wallbit: ${input.title}`,
    "",
    `Por qué ahora: ${input.rationale}`,
    "",
    "Riesgos:",
    ...input.risks.map((r) => `• ${r}`),
    "",
  ...
  if (input.source?.title) {
    lines.push("", `Fuente: ${input.source.title}`);
    if (input.source.url) lines.push(input.source.url);
  }
  lines.push(
    "",
    `Acción preparada (${input.kind}): Bs ${input.amountBobs.toLocaleString("es-BO")}.`,
    "Nada se ejecuta sin tu confirmación.",
  );
  return lines.join("\n");
}
...
async function prepareDigestForUser(userId: string): Promise<string | null> {
  ...
  if (kind === "wallbit_convert") {
    ...
    // applyMicrosaving lee payload.source como enum de aporte.
  ...
  if (profile?.telegramUserId) {
    const text = formatDigestTelegramText({
      title,
      rationale,
      risks,
      benefits,
      source: newsSource
        ? { title: newsSource.title, url: newsSource.url }
        : undefined,
      amountBobs,
      kind,
    });
    await notifyChannel("telegram", String(profile.telegramUserId), text, {
      buttons: [
